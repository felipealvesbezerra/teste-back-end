...
let AuthenticateUserService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (!user) {
      throw new _AppError.default('Combinação de e-mail e senha incorretos.', 401);
    }
    ...
    if (!user.verify) {
      throw new _AppError.default('O usuário ainda não foi ativado, verifique seu e-mail.');
    }
